...
| **7:30–15:30** | Multi-specification BCC showcase | Spend roughly two minutes each on GraphQL, gRPC, and AsyncAPI. Show the optional GraphQL `status` argument becoming mandatory, `orderId` integer → string, and string `status` → structured event object, along with each scenario, BCC result, affected contract location, and a brief remediation option. Use the remaining time to compare the examples. |
